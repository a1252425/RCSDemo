...
typedef struct BSStackFrameEntry{
    const struct BSStackFrameEntry *const previous; //X29+X30
    const uintptr_t return_address; //X30
} BSStackFrameEntry;
...
+ (NSString *)bs_backtraceOfAllThread {
    ...
    kern_return_t kr = task_threads(this_task, &threads, &thread_count);    //取所有线程的栈时不需要NSThread到mach层的转换
    if(kr != KERN_SUCCESS) {
...
NSString *_bs_backtraceOfThread(thread_t thread) {
    ...
    _STRUCT_MCONTEXT machineContext;    //从中获取栈帧结构体
    if(!bs_fillThreadStateIntoMachineContext(thread, &machineContext)) {
        return [NSString stringWithFormat:@"Fail to get information about thread: %u", thread];
    }
    
    const uintptr_t instructionAddress = bs_mach_instructionAddress(&machineContext);   //最外侧调用栈的 pc
    backtraceBuffer[i] = instructionAddress;
    ++i;
    
    uintptr_t linkRegister = bs_mach_linkRegister(&machineContext); //上层调用栈的 lr
    if (linkRegister) {
    ...
    bs_symbolicate(backtraceBuffer, symbolicated, backtraceLength, 0);  //符号化地址
...
thread_t bs_machThreadFromNSThread(NSThread *nsthread) {
    ...
    if ([nsthread isMainThread]) {  //主线程直接匹配
        return (thread_t)main_thread_id;
    }
    
    for (int i = 0; i < count; ++i) {   //子线程匹配前面设置的时间戳name
...
void bs_symbolicate(const uintptr_t* const backtraceBuffer,
    ...
    //lr处理时去除地址中的指针标签，以及减 1 获取 lr 前一个地址，即函数调用处
    for(; i < numEntries; i++) {
            ...
            for(uint32_t iSym = 0; iSym < symtabCmd->nsyms; iSym++) {
                ...
                if(symbolTable[iSym].n_value != 0) {
                    ...
                    if((addressWithSlide >= symbolBase) &&
                       (currentDistance <= bestDistance)) { //找到和地址最接近的符号
...
uint32_t bs_imageIndexContainingAddress(const uintptr_t address) {
    ...
    //segment前后地址匹配目标地址，包含在中间则说明在匹配镜像中
